...
export default function ShopDialog({ isOpen, onClose, playerPTS, onGetPTS, shipLevel }: ShopDialogProps) {
  ...
  const shopItems: ShopItem[] = [
    {
      id: 'extra-game',
      name: 'Extra Game',
      description: 'Add one more game — play anytime',
      cost: prices.attempt,
    ...
    },
    {
      id: 'booster',
      name: 'Booster',
      description: 'Double fire rate for a limited time',
      cost: prices.booster,
    ...
    },
    {
      id: 'ship-upgrade',
      name: 'Ship Upgrade',
      description: 'Level up your ship for enhanced firepower',
      cost: prices.ship,
  ...
  const handlePurchase = () => {
    if (selectedItem === 'extra-game' && selectedItemData && playerPTS >= selectedItemData.cost && gamesAvailable < 3) {
      return buyAttempt({ args: [], gasLimit: { increaseGas: 30 } })
        .then(() => {
    ...
    if (selectedItem === 'ship-upgrade' && canUpgrade && selectedItemData && playerPTS >= selectedItemData.cost) {
      return buyShip({ args: [], gasLimit: { increaseGas: 30 } })
        .then(() => {
    ...
    if (selectedItem === 'booster' && selectedItemData && playerPTS >= selectedItemData.cost) {
      return buyBooster({ args: [], gasLimit: { increaseGas: 30 } })
        .then(() => {
