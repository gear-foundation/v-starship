...
export class SailsDecoder {
  ...
  decodeInput<T>({
    ...
    const params = this._program.services[service].functions[method].decodePayload<T>(payload);
  ...
  decodeOutput<T>({
    ...
    const service = this.service(payload);
    const method = this.method(payload);
    const _payload = this._program.services[service].functions[method].decodeResult<T>(payload);
  ...
  decodeQueryOutput<T>(service: string, fn: string, data: string): T {
    return this._program.services[service].queries[fn].decodeResult<T>(data as `0x${string}`);
  }
}
